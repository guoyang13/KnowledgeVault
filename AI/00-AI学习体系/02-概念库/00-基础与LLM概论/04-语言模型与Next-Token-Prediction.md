...
| **Transformer LM** | Self-Attention 直接看全局（窗口内）上下文 | 成为 LLM 主流，见 [[AI/00-AI学习体系/02-概念库/00-基础与LLM概论/06-从RNN-CNN到Attention简史|从 RNN/CNN 到 Attention 简史]] |

## 为什么「预测下一个词」能变「聪明」

- 要预测得好，模型必须隐式学习**语法、事实关联、常识、代码模式**等
- 规模与数据上去后，**下游任务**可通过提示（ICL）或微调（SFT）激发，而不必为每个任务单独建模型
- 这是 **「预训练 + 对齐」** 范式的根基：见 [[AI/00-AI学习体系/02-概念库/02-训练与推理/01-训练阶段与对齐方法|训练阶段与对齐方法]]
...
- [[AI/00-AI学习体系/02-概念库/00-基础与LLM概论/02-神经网络与反向传播|神经网络与反向传播]]
