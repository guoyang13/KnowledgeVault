...
> 2026-07 更新：Embedding 正在从文本扩展到多模态统一空间，图片、视频、音频、PDF 与文本可以被映射到同一检索体系中。做 RAG 时要把“文本召回”升级为“对象召回 + rerank + 引用定位”。
...
更基础的表示学习见 [[AI/00-AI学习体系/02-概念库/00-基础与LLM概论/03-表示学习-从词嵌入到上下文|表示学习-从词嵌入到上下文]]。
...
对应 [[AI/00-AI学习体系/02-概念库/04-RAG进阶/01-RAG基础|RAG基础]] 的 Indexing 和 Retrieval 阶段。
...
- **开源/开放模型**：BGE、E5、GTE、Jina Embeddings、Qwen3 Embedding。
- **多模态 Embedding**：Gemini Embedding 2 等统一文本、图像、视频、音频、PDF 表示的模型方向。
...
- **多模态只做 OCR**：图片/PDF/视频不一定只转文字，有时需要直接做视觉或多模态向量检索。
- **引用定位缺失**：多模态 RAG 更需要记录页码、帧号、区域坐标、原始文件路径，方便答案溯源。
...
- [[AI/00-AI学习体系/02-概念库/00-基础与LLM概论/03-表示学习-从词嵌入到上下文|表示学习-从词嵌入到上下文]]
