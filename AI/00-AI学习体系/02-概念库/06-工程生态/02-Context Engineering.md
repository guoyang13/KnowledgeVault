...
4. **Memory**：长期记忆召回（见 [[AI/00-AI学习体系/02-概念库/03-Agent系统/08-Agent Memory|Agent Memory]]）。
5. **RAG**：当下任务相关的检索片段（见 [[AI/00-AI学习体系/02-概念库/04-RAG进阶/01-RAG基础|RAG基础]]）。
6. **History**：消息历史、工具调用结果。
7. **当前任务输入**：用户最新消息 + 相关文件。

## 关键问题
- **预算分配**：上下文窗口有限，哪些必须进、哪些可压缩、哪些可省略。
- **顺序**：把"不变的"放前面以命中 [[AI/00-AI学习体系/02-概念库/01-模型层/07-上下文与KV Cache|Prefix Cache]]。
