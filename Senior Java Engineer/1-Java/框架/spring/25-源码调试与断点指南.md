...
> 导航：[[00-Spring-Bean加载-学习导航]] · **下篇 16–25** · 断点指南 · 配套：[[17-Bean加载原理与源码阅读路径]]

## 前置条件

- 本地已 clone Spring Framework 源码：`/Users/guoyang/IdeaProjects/spring/spring-framework`
- IDE 中打开上述工程，确保能跳转源码
- 理论背景：[[16-IoC与DI核心概念]]、[[17-Bean加载原理与源码阅读路径]]
...
new AnnotationConfigApplicationContext(Config.class)
  ...
  │     └─ refresh()                            // ★ 容器启动总入口
  │
  └─ AbstractApplicationContext#refresh()         // spring-context/.../AbstractApplicationContext L588
...
> `register()` / Reader 构造 / `@Component` 三条注册路径详解 → [[07-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]]
...
AbstractBeanFactory#getBean(name)
  └─ AbstractBeanFactory#doGetBean(name, ...)     // ★ spring-beans/.../AbstractBeanFactory L242
...
| 顺序 | 类#方法 | 文件 | 看什么 |
|:----:|---------|------|--------|
| 1 | `AbstractApplicationContext#refresh` | spring-context L588 | 启动 12 步 |
...
| 4 | `AbstractBeanFactory#doGetBean` | spring-beans L242 | 缓存 vs 新建 |
| 5 | `AbstractAutowireCapableBeanFactory#doCreateBean` | spring-beans | 创建三阶段 |
| 6 | `populateBean` | spring-beans L1403 | @Autowired |
| 7 | `DefaultListableBeanFactory#doResolveDependency` | spring-beans L1722 | 依赖解析 |
...
4. 对照 [[17-Bean加载原理与源码阅读路径]] 8 步清单逐文件阅读
5. 扩展点专题：[[19-IoC扩展点三部曲对照]] → 16 → 17 → 18 → 20
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[24-Bean销毁机制详解]] | [[00-Spring-Bean加载-学习导航|学习导航（完结）]] |
