...
> 导航：[[00-Spring-Bean加载-学习导航]] · **上篇 01–15** · 注册层
>
> 前置：[[07-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]] · [[06-元数据层-BeanDefinition三兄弟详解]]
>
> 关联：[[09-容器层-BeanFactory与Registry详解]]（BeanFactory 接口体系）· [[14-工厂Bean-BeanFactory与FactoryBean的区别]] · [[15-工厂Bean-FactoryBean接口体系详解]] · [[10-Context层-ApplicationContext详解]]
>
...
源码（`BeanDefinitionRegistry.java` L33-46）：
...
```java
// DefaultListableBeanFactory.java L216-246
...
实现位置：`DefaultListableBeanFactory.registerBeanDefinition()`（L1296）
...
`getBeanNamesForType(UserService.class)` 核心逻辑（`doGetBeanNamesForType`，L627）：
...
`@Autowired` 注入最终调用 `doResolveDependency()`（L1665）：
...
**findAutowireCandidates() 三步兜底**（L1973）：
...
`ChildBeanDefinition` 可继承 parent 的属性。创建 Bean 时用的是**合并后的 `RootBeanDefinition`**，不是 `beanDefinitionMap` 中的原始定义。详见 [[06-元数据层-BeanDefinition三兄弟详解]]。
...
| 优先级 | 方法 | 文件位置 | 回答的问题 |
|:------:|------|---------|-----------|
| ⭐⭐⭐ | `registerBeanDefinition()` | DLBF L1296 | 定义怎么进容器？ |
| ⭐⭐⭐ | `preInstantiateSingletons()` | DLBF L1111 | 非 lazy 单例何时创建？ |
| ⭐⭐ | `doGetBeanNamesForType()` | DLBF L627 | 按类型怎么找 Bean？ |
| ⭐⭐ | `doResolveDependency()` | DLBF L1665 | @Autowired 怎么解析？ |
| ⭐⭐ | `resetBeanDefinition()` | DLBF L1411 | 覆盖定义后缓存怎么清？ |
| ⭐ | `freezeConfiguration()` | DLBF L1005 | 配置冻结做了什么？ |

> BeanFactory 接口体系、doGetBean 完整链路 → [[09-容器层-BeanFactory与Registry详解]]
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[07-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]] | [[09-容器层-BeanFactory与Registry详解]] |
