...
type: canonical
status: reviewed
topic: Spring ApplicationContext
source_version: 6.2.x
---

# ApplicationContext 详解

> 导航：[[00-Spring-Framework核心机制-学习导航]] · **20 · 结构与元数据** · Context层
>
> 前置：[[4-容器层-BeanFactory接口体系详解]] · [[3-注册层-BeanDefinitionRegistry与DefaultListableBeanFactory详解]]
>
> 关联：[[3-注册层-BeanDefinitionRegistry与DefaultListableBeanFactory详解#五、与 ApplicationContext 的关系]]
>
> 本地源码：
...
> 详细对比见 [[4-容器层-BeanFactory接口体系详解#1.6 BeanFactory vs ApplicationContext]]。
...
> 详见 [[3-注册层-BeanDefinitionRegistry与DefaultListableBeanFactory详解#五、与 ApplicationContext 的关系]]。
...
> Reader / Scanner 构造链路、`register` vs `scan`、@Component 注册三条路径 → [[2-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]]
...
两者最终都汇入同一个 `refresh` 流程。详见 [[2-Bean加载原理与源码阅读路径#第 3 步：选一个「定义加载」入口]]。

---

## 核心生命周期：`refresh`

ApplicationContext 的启动核心是 `AbstractApplicationContext.refresh`：
...
> **12 阶段逐步解析（每个子方法中文说明）** → [[3-refresh方法详解]]  
> 完整阅读路径见 [[2-Bean加载原理与源码阅读路径]]。
...
> 完整 14 步初始化顺序见 [[4-容器层-BeanFactory接口体系详解#1.5 Bean 生命周期（BeanFactory 定义的标准顺序）]]。
...
| `refresh` | 容器启动总入口 | `AbstractApplicationContext` |
| `finishBeanFactoryInitialization` | 预实例化非 lazy 单例 | `AbstractApplicationContext` |
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[4-容器层-BeanFactory接口体系详解]] | [[1-扩展点层-BeanFactoryPostProcessor详解]] |

> 上篇继续：[[1-扩展点层-BeanFactoryPostProcessor详解]] → [[2-扩展点层-BeanPostProcessor详解]] → [[3-生命周期层-Aware体系详解]]；下篇机制：[[1-IoC与DI核心概念]] → [[2-Bean加载原理与源码阅读路径]]
...
- [x] `GenericApplicationContext` 与 `AnnotationConfigApplicationContext` 构造差异 → [[2-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]]
