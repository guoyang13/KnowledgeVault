...
> 导航：[[00-Spring-Framework核心机制-学习导航]] · **30 · 扩展点与生命周期** · 生命周期层 · Aware 接口体系
>
> 前置：[[4-接口地图-IoC与DI重要接口大全#4. 生命周期协议]] · [[2-扩展点层-BeanPostProcessor详解]] · [[5-Context层-ApplicationContext详解]]
>
> 关联：[[7-IoC扩展点三部曲对照]] · [[5-依赖注入实现原理]]（Aware vs DI）
>
...
> **比喻**：新员工（Bean）入职时在表格上勾选「我需要工牌、门禁卡、公司通讯录」（实现对应的 `XxxAware` 接口），HR（容器）在正式上岗（初始化）前主动把这些东西一件件发到他手里（回调 `setXxx`）。员工不用自己跑去申领——是公司**发**给他的；而且发的都是「公司自带的东西」（基础设施），不是他跟同事借的业务资料（那属于 `@Autowired`）。

> 速查版见 [[4-接口地图-IoC与DI重要接口大全#4. 生命周期协议]]；本篇是 Aware 的权威主文档。
...
**无必然关系**。Aware 是生命周期基础设施回调；FactoryBean 是「工厂产 P」；DDD 编译时接口解耦见 [[02-DDD分层的编译时依赖与Spring运行时装配]]。
...
详见 [[2-扩展点层-BeanPostProcessor详解#三、在 Bean 生命周期中的位置]]、[[2-Bean加载原理与源码阅读路径]]。
...
**位置**：`AbstractAutowireCapableBeanFactory.initializeBean` 最开头  
...
**位置**：`postProcessBeforeInitialization` 内
...
详见 [[5-Context层-ApplicationContext详解]] 的 `prepareBeanFactory`。
...
→ `resolveDependency` 详解见 [[5-依赖注入实现原理]]。
...
→ 详见 [[5-依赖注入实现原理]] · [[2-扩展点层-BeanPostProcessor详解]]
...
→ 详见 [[5-Context层-ApplicationContext详解]] · [[3-refresh方法详解]]
...
用途：被 `@Import` 导入的配置类，可以知道「是谁导入了我」，读取导入方的注解元数据。详见 [[1-扩展点层-BeanFactoryPostProcessor详解#八、ConfigurationClassPostProcessor 深入]]。
...
这就是 `ApplicationContextAwareProcessor` 的同款模式。详见 [[2-扩展点层-BeanPostProcessor详解#十三、自定义 BPP]]。
...
| 扩展点 | 操作对象 | 详见 |
|--------|----------|------|
| BFPP | BeanDefinition | [[1-扩展点层-BeanFactoryPostProcessor详解]] |
| BPP | Bean 实例 | [[2-扩展点层-BeanPostProcessor详解]] |
...
| 11 | 对比 [[2-扩展点层-BeanPostProcessor详解]] | Aware 是 BPP BeforeInit 的典型应用 |

建议在 `invokeAwareMethods` 和 `ApplicationContextAwareProcessor.postProcessBeforeInitialization` 上打断点，配合 [[1-源码调试与断点指南]] 跟栈。
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[2-扩展点层-BeanPostProcessor详解]] | [[5-工厂Bean-BeanFactory与FactoryBean的区别]] |
