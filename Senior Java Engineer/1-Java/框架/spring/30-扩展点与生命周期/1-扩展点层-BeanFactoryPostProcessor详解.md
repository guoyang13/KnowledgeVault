...
type: canonical
status: reviewed
topic: Spring BeanFactoryPostProcessor
source_version: 6.2.x
---

# BeanFactoryPostProcessor 详解

> 导航：[[00-Spring-Framework核心机制-学习导航]] · **30 · 扩展点与生命周期** · 扩展点层 · BFPP
>
> 前置：[[4-接口地图-IoC与DI重要接口大全#3. 容器扩展点]] · [[5-Context层-ApplicationContext详解]] · [[1-元数据层-BeanDefinition三兄弟详解]]
>
> 关联：[[7-IoC扩展点三部曲对照]] · [[2-扩展点层-BeanPostProcessor详解]] · [[3-refresh方法详解]]
>
...
→ BPP 完整版：[[2-扩展点层-BeanPostProcessor详解]]
...
**文件**：`PostProcessorRegistrationDelegate.invokeBeanFactoryPostProcessors`
...
与 BPP 相同（详见 [[2-扩展点层-BeanPostProcessor详解#十一、排序规则]]）：
...
**为什么应使用 static？**（Spring 官方强烈推荐，见 `BeanFactoryPostProcessor.java`）
...
**注意：不是「一定会有部分 Bean 无法完整后处理」**——PostProcessor 链注册完成后创建的业务 Bean 通常不受影响。详见 [[2-扩展点层-BeanPostProcessor详解#13.2 @Configuration 中注册 — 应使用 static]] 中的情况对照表。
...
| 9 | 对比 [[2-扩展点层-BeanPostProcessor详解]] | BFPP vs BPP 分工 |

建议在 `ConfigurationClassPostProcessor.postProcessBeanDefinitionRegistry` 上打断点，配合 [[1-源码调试与断点指南]]。
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[5-Context层-ApplicationContext详解]] | [[2-扩展点层-BeanPostProcessor详解]] |
...
- [ ] BFPP vs BPP 对照 — 与 [[2-扩展点层-BeanPostProcessor详解]] 联合复习
