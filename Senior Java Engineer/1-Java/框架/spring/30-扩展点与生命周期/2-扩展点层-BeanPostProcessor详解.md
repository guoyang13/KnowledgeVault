...
type: canonical
status: reviewed
topic: Spring BeanPostProcessor
source_version: 6.2.x
---

# BeanPostProcessor 详解

> 导航：[[00-Spring-Framework核心机制-学习导航]] · **30 · 扩展点与生命周期** · 扩展点层 · BPP
>
> 前置：[[1-扩展点层-BeanFactoryPostProcessor详解]] · [[4-容器层-BeanFactory接口体系详解]] · [[4-接口地图-IoC与DI重要接口大全#3. 容器扩展点]]
>
> 关联：[[7-IoC扩展点三部曲对照]] · [[5-依赖注入实现原理]] · [[8-Spring-AOP代理创建详解]]
>
...
> 速查版见 [[4-接口地图-IoC与DI重要接口大全#3. 容器扩展点]]；本篇是完整深入版。
...
结合 `doCreateBean` 的完整时序（详见 [[2-Bean加载原理与源码阅读路径]]）：
...
→ 销毁登记、`requiresDestruction`、为何 Context.close 就需要销毁：[[9-Bean 销毁机制详解]]
...
与 [[5-依赖注入实现原理]] 的关系：`postProcessProperties`（介入点 ④）是 `@Autowired` 字段/方法注入的入口，在 `invokeInitMethods` 之前完成。
...
`AutowiredAnnotationBeanPostProcessor.postProcessProperties` 扫描 `@Autowired`、`@Value`、`@Inject`，完成字段/方法注入。详见 [[5-依赖注入实现原理]]。
...
详见 [[5-Context层-ApplicationContext详解]] 的 `refresh` 步骤。
...
→ 代理创建条件、`wrapIfNecessary`、JDK/CGLIB 决策、常见注解是否触发代理：详见 [[8-Spring-AOP代理创建详解]]
...
**为什么应使用 static？**（Spring 官方强烈推荐，见 `BeanPostProcessor.java`）
...
→ BFPP 同理，见 [[1-扩展点层-BeanFactoryPostProcessor详解#6.2 @Configuration 中注册 — 应使用 static]]
...
建议在 `populateBean` 和 `initializeBean` 上打断点，观察 BPP 链的实际调用顺序。配合 [[1-源码调试与断点指南]] 跟栈。
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[1-扩展点层-BeanFactoryPostProcessor详解]] | [[3-生命周期层-Aware体系详解]] |
...
- [x] `AbstractAutoProxyCreator` — AOP 代理创建时机 → [[8-Spring-AOP代理创建详解]]
- [ ] 循环依赖 — `SmartInstantiationAwareBeanPostProcessor.getEarlyBeanReference`（与 [[8-Spring-AOP代理创建详解#六、代理在 doCreateBean 中的时机与替换]] 交叉）
- [ ] `AutowiredAnnotationBeanPostProcessor` — 与 [[5-依赖注入实现原理]] 对照精读
- [x] Aware 体系 → [[3-生命周期层-Aware体系详解]]
