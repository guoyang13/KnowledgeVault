...
→ 详见 [[#八、完整调用链]] · [[14-工厂Bean-BeanFactory与FactoryBean的区别#getBean 时的容器处理]]
...
→ 跟栈：`AbstractBeanFactory.getObjectForBeanInstance()` · `FactoryBeanRegistrySupport.getObjectFromFactoryBean()`（[[25-源码调试与断点指南]]）
...
与 **DDD 普通 Repository** 对比：`MyBatisOrderRepository implements OrderRepository` 编译期有实现类 → `@Bean` / `@Repository` 直接注册 **P**，不需要 F。见 [[100-Q&A/DDD分层-编译时运行时与Spring装配]]。
...
**记忆**：FactoryBean 抽象 **「造 Bean 的 Bean」**，不是 **「被增强的 Bean」**。AOP 详解见 [[22-Spring-AOP代理创建详解]]；动态代理概念见 [[100-Q&A/动态代理是什么]]。
