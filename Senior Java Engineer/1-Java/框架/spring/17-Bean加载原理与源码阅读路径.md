...
> 导航：[[00-Spring-Bean加载-学习导航]] · **下篇 16–25** · Bean 加载主线 · 前置：[[16-IoC与DI核心概念]]、[[01-注解入门-配置类与组件类]]、[[02-注解入门-Configuration与Service等注解区别]]
>
...
> 三兄弟职责、注解场景下的产生路径、合并流程 → [[06-元数据层-BeanDefinition三兄弟详解]]
...
> 接口体系、核心数据结构、`registerBeanDefinition` / `resolveDependency` 完整流程 → [[08-注册层-BeanDefinitionRegistry与DefaultListableBeanFactory详解]]
>
> BeanFactory 接口体系、doGetBean 流程 → [[09-容器层-BeanFactory与Registry详解]]
>
> ApplicationContext 详解 → [[10-Context层-ApplicationContext详解]]
>
> BeanFactory vs FactoryBean、FactoryBean 接口体系 → [[14-工厂Bean-BeanFactory与FactoryBean的区别]] · [[15-工厂Bean-FactoryBean接口体系详解]]
...
> `register()` 与 `scan()` 入口、`@Component` 三条注册路径 → [[07-元数据层-AnnotatedBeanDefinitionReader与组件注册详解]]
...
> **逐步解析（12 阶段、每个子方法）** → [[18-refresh方法详解]]  
> ApplicationContext 完整说明 → [[10-Context层-ApplicationContext详解]]
...
→ 扩展点分工总览：[[19-IoC扩展点三部曲对照]] · BFPP [[11-扩展点层-BeanFactoryPostProcessor详解]] · BPP [[12-扩展点层-BeanPostProcessor详解]]
...
**文件**：`spring-beans/.../support/AbstractBeanFactory.java`（L201）
...
```text
createBean(beanName, mbd, args)                    // L492
├── resolveBeanClass()
├── resolveBeforeInstantiation()                   // AOP 等可提前返回代理
└── doCreateBean(beanName, mbd, args)              // L560
      ├── createBeanInstance()                     // 实例化
      ├── applyMergedBeanDefinitionPostProcessors()
      ├── addSingletonFactory()                    // 三级缓存，解决循环依赖
      ├── populateBean()                           // 属性注入 / @Autowired ← DI
      ├── initializeBean()                         // Aware 回调、init、BPP
      └── registerDisposableBeanIfNecessary()      // 销毁登记 → [[24-Bean销毁机制详解]]
      // 循环依赖：addSingletonFactory() 在 populateBean 之前 → [[21-循环依赖与三级缓存详解]]
```

`createBeanInstance()` 的决策顺序（L1186 附近）：
...
当使用默认无参构造器时，`createBeanInstance()` 会调用 `instantiateBean()`（L1347）：
...
```text
finishBeanFactoryInitialization()
  → DefaultListableBeanFactory.preInstantiateSingletons()   // L1111
    → preInstantiateSingleton(beanName, mbd)
      → if (!mbd.isLazyInit()) instantiateSingleton(beanName)
            → getBean(beanName)
```

> 注册了但没人用的 Bean 也会走这条路径，详见 [[100-Q&A/未被使用的类Spring如何处理]]。
...
4. **主流程跟完**再进 [[12-扩展点层-BeanPostProcessor详解]]、循环依赖、`ConfigurationClassEnhancer` 等旁支

详细断点清单和三次跟栈路线见 [[25-源码调试与断点指南]]。
...
- [x] `BeanPostProcessor` — 在 `populateBean` / `initializeBean` 中的介入点 → [[12-扩展点层-BeanPostProcessor详解]]
- [x] `BeanFactoryPostProcessor` — `invokeBeanFactoryPostProcessors()` → [[11-扩展点层-BeanFactoryPostProcessor详解]]
