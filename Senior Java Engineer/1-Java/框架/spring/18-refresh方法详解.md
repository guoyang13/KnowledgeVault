...
> 导航：[[00-Spring-Bean加载-学习导航]] · **下篇 16–25** · refresh 机制
>
> 源码：`spring-context/.../support/AbstractApplicationContext.java`（`refresh()` 及全部子方法）
>
> 前置：[[17-Bean加载原理与源码阅读路径]] · [[10-Context层-ApplicationContext详解]]
>
> 关联：[[19-IoC扩展点三部曲对照]] · [[11-扩展点层-BeanFactoryPostProcessor详解]] · [[12-扩展点层-BeanPostProcessor详解]] · [[20-依赖注入实现原理]] · [[13-生命周期层-Aware体系详解]]
...
> ApplicationContextAware 等回调的完整体系 → [[13-生命周期层-Aware体系详解]]
...
→ 详见 [[11-扩展点层-BeanFactoryPostProcessor详解]] · [[19-IoC扩展点三部曲对照]]
...
→ 详见 [[12-扩展点层-BeanPostProcessor详解]]
...
└─ preInstantiateSingletons()            ★ 核心
      └─ getBean(name) → doGetBean() → doCreateBean()
            ...
            ├─ populateBean()             DI ← [[20-依赖注入实现原理]]
            └─ initializeBean()           Aware + @PostConstruct + BPP
```

→ 详见 [[17-Bean加载原理与源码阅读路径#第 7 步]] · [[100-Q&A/未被使用的类Spring如何处理]]
...
→ 销毁机制详解（登记 vs 执行、DisposableBeanAdapter）：[[24-Bean销毁机制详解]]
...
| 上一篇 | 下一篇 |
|--------|--------|
| [[17-Bean加载原理与源码阅读路径]] | [[19-IoC扩展点三部曲对照]] |
