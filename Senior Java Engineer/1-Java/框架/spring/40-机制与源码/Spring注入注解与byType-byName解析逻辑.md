...
status: reviewed
topic: Spring dependency resolution
source_version: 6.2.x
---

# Spring 注入注解与 byType / byName 解析逻辑

> 导航：[[00-Spring-Framework核心机制-学习导航]] · **40 · 机制与源码** · 依赖注入 · 候选解析
>
> 前置：[[5-依赖注入实现原理]] · [[Spring依赖注入形式分类与Demo]]
>
> 本文只深入候选解析规则和源码分支；依赖注入的定义、生命周期位置和总体调用链以 [[5-依赖注入实现原理]] 为准。
>
> 源码：
...
**源码位置：** `DefaultListableBeanFactory.java`
...
**`determineAutowireCandidate` 优先级：**
...
**源码：** `QualifierAnnotationAutowireCandidateResolver.getSuggestedName`
...
**源码：** `CommonAnnotationBeanPostProcessor.ResourceElement`
...
| 观察点 | 稳定定位 |
| --- | --- |
| 依赖解析主流程 | `DefaultListableBeanFactory#doResolveDependency` |
| 依赖名称快速匹配 | `DefaultListableBeanFactory#doResolveDependency` |
| 类型候选查找 | `DefaultListableBeanFactory#findAutowireCandidates` |
| 多候选消歧 | `DefaultListableBeanFactory#determineAutowireCandidate` |
| `@Qualifier` 建议名称 | `QualifierAnnotationAutowireCandidateResolver#getSuggestedName` |
| 候选转为 Bean | `DependencyDescriptor#resolveCandidate` |
| `@Resource` 注入 | `CommonAnnotationBeanPostProcessor#autowireResource` |
| XML byName | `AbstractAutowireCapableBeanFactory#autowireByName` |
| XML byType | `AbstractAutowireCapableBeanFactory#autowireByType` |
