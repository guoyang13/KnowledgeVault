...
type: canonical
status: reviewed
topic: Spring circular dependency / singleton caches
source_version: 6.2.x
---

# 循环依赖与三级缓存详解

> 导航：[[00-Spring-Framework核心机制-学习导航]] · **40 · 机制与源码** · 循环依赖
>
> 源码：
...
>
> 前置：[[5-依赖注入实现原理]] · [[2-Bean加载原理与源码阅读路径]]
>
> 关联：[[8-Spring-AOP代理创建详解]] · [[2-扩展点层-BeanPostProcessor详解]] · [[1-源码调试与断点指南]]
...
**定义位置：** `DefaultSingletonBeanRegistry.java`
...
**位置：** `DefaultSingletonBeanRegistry.java`
...
├─ addSingletonFactory             ★ 三级，必须在 populateBean 之前
├─ populateBean                    @Autowired → nested getBean
├─ initializeBean                  init + AOP
└─ 早期引用 vs 最终对象一致性校验（，仅 factory 执行过才生效）
```
...
| 路径 | 条件 | 代码 |
|------|------|------|
| **A** 字段名=Bean名 | `@Autowired B b`，Bean 名也是 `"b"` | `doResolveDependency` Step3 → `getBean("b")` |
| **B** 按类型匹配 | 字段名≠Bean名，或唯一实现 | `findAutowireCandidates` → `resolveCandidate` → `getBean` |

→ 构造器注入不走此路径：[[5-依赖注入实现原理#构造器注入 DI 地图]]
...
B 注入 A 时，三级 factory 回调 `getEarlyBeanReference`：
...
**不一致风险：** 早期注入 raw A，最终 proxy A → `doCreateBean` 可能抛异常。  
→ 完整 raw/proxy 替换机制：[[8-Spring-AOP代理创建详解#六、代理在 doCreateBean 中的时机与替换]]
...
| 观察点 | 稳定定位 |
| --- | --- |
| 三级缓存字段 | `DefaultSingletonBeanRegistry` 的 singleton cache fields |
| 注册三级工厂 | `DefaultSingletonBeanRegistry#addSingletonFactory` |
| 三级转二级 | `DefaultSingletonBeanRegistry#getSingleton` |
| 写入一级缓存 | `DefaultSingletonBeanRegistry#addSingleton` |
| 一级缓存命中 | `AbstractBeanFactory#doGetBean` |
...
| 注册早期引用与最终一致性校验 | `AbstractAutowireCapableBeanFactory#doCreateBean` |
| 获取 Bean 早期引用 | `AbstractAutowireCapableBeanFactory#getEarlyBeanReference` |
| 创建 AOP 早期代理 | `AbstractAutoProxyCreator#getEarlyBeanReference` |

### 调试断点

| 顺序 | 位置 | 看什么 |
|:----:|------|--------|
| 1 | `doCreateBean` | 是否 `addSingletonFactory` |
| 2 | `populateBean` | 是否进入 BPP |
| 3 | `resolveDependency` | 是否 nested `getBean` |
| 4 | `getSingleton` | 三级→二级（**仅有环时命中**） |
| 5 | `getEarlyBeanReference` | AOP 早期代理 |
| 6 | `doCreateBean` | 早期 vs 最终一致性 |
